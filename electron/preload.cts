...
export type VaultMember = { address: string; label: string | null; invitedBy: string | null; joinedAt: number };
export type VaultInvitation = { address: string; label: string | null; invitedBy: string; createdAt: number; inviterIsActive: boolean };
...
contextBridge.exposeInMainWorld("ipiDesktop", Object.freeze({
  ...
  reviewVaultCreate: (label: string): Promise<VaultReview> => ipcRenderer.invoke("vault:review-create", label),
  reviewVaultInvite: (contractAddress: string, cardAddress: string, label: string): Promise<VaultReview> => ipcRenderer.invoke("vault:review-invite", contractAddress, cardAddress, label),
...
export type VaultReview = { reviewId: string; expiresAt: string; action: "create" | "invite" | "accept" | "cancel" | "remove" | "transfer"; signer: string; contractAddress: string | null; target: string | null; label: string | null; amount: string | null; fee: string; controllerBalance: string; vaultBalance: string | null; chainId: string; codeId: string };
